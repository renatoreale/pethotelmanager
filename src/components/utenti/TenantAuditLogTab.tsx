...
import { Eye, Loader2, RefreshCw, Link2 } from "lucide-react";
import { useAuth } from "@/hooks/useAuth";
import { useAuditLog, useAuditLogSiblings, type AuditLogEntry } from "@/hooks/useAuditLog";
...
// Blocco 34 (Audit log): stessa "Storico Modifiche" già disponibile per il
// super-admin (AuditLogTab.tsx), ma con vista fissata sulla propria pensione
// (le RLS di audit_log lo garantiscono comunque: un utente vede solo le
// righe del proprio tenant, o tutte se admin) e SENZA la funzione di
// ripristino, che resta un'azione riservata al super-admin di piattaforma
// (admin-restore-audit-record verifica il ruolo admin indipendentemente
// da questa UI). Componente separato invece di riadattare quello admin:
// quello usa useAllTenants/useAllUsers (dati cross-tenant non accessibili
// a titolare/manager), qui basta il proprio elenco utenti (useUsers).
export function TenantAuditLogTab() {
  ...
  const { data: entries, isLoading, refetch, isFetching } = useAuditLog({
    tableName, operation, recordId, tenantId: profile?.tenant_id ?? "", limit,
  });
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <p className="text-sm text-muted-foreground">
          Chi ha creato, modificato o eliminato cosa, e quando, all'interno della tua pensione.
        </p>
      ...
      <Dialog open={!!detail} onOpenChange={(open) => { if (!open) setDetail(null); }}>
        <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              {detail && `${formatDate(detail.created_at)} · ${userLabel(detail.user_id)} (${detail.user_role ?? "—"})`}
            </DialogDescription>
          </DialogHeader>
          {detail && (
