...
serve(async (req) => {
  ...
  try {
    ...
    const callMysql = async (payload: Record<string, unknown>) => {
      ...
      const text = await mysqlRes.text();
      return {
        ok: mysqlRes.ok,
    ...
    if (action === "confirm_by_token") {
      ...
      const mysqlResult = await callMysql({ action: "confirm_by_token", token });

      if (!mysqlResult.ok) {
        console.error("MySQL confirm error:", mysqlResult.bodyText);
        return new Response(JSON.stringify({ error: "Errore nella conferma della richiesta" }), {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }

      return new Response(mysqlResult.bodyText, {
        status: 200,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    const mysqlResult = await callMysql({
      action: "insert",
    ...
    if (!mysqlResult.ok) {
      console.error("MySQL insert error:", mysqlResult.bodyText);
