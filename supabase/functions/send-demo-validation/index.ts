...
serve(async (req) => {
  ...
  try {
    ...
    // ── CONFIRM BY TOKEN (legacy — kept for backward compat) ──
    ...
    // ── INSERT NEW LEAD ───────────────────────────────────────
    const { email, firstName, lastName, phone, leadType, pensioneName, message, baseUrl } = body;
    ...
    const siteUrl = baseUrl || "https://pethotelmanager.com";
    const redirectTo = `${siteUrl}/reset-password`;

    // Salva il lead
    await adminClient.from("demo_leads").insert({
    ...
    // Crea l'utente Supabase (ignora errore se già esistente)
    await adminClient.auth.admin.createUser({
      email: cleanEmail,
      email_confirm: true,
      user_metadata: { is_trial: true, full_name: fullName },
    });

    // Genera link per impostare la password
    ...
    if (linkError || !linkData?.properties?.action_link) {
      ...
      return new Response(JSON.stringify({ error: "Errore nella creazione del link di accesso" }), {
    ...
    // Invia unica email con link imposta password
      ...
      const html = `
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:32px 24px;">
          ...
          <p style="color:#333;font-size:16px;line-height:1.6;">
            Grazie per aver richiesto la prova gratuita di PetHotelManager!<br/>
            Clicca il pulsante qui sotto per impostare la tua password e accedere subito:
          </p>
          ...
          <p style="color:#999;font-size:13px;">Il link è valido per 24 ore.</p>
